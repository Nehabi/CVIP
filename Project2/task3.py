...
import numpy as np
structuring_ele = np.array([[1,1,1],[1,1,1],[1,1,1]])
...
def morph_erode(img):
    ...
    erode_img = np.zeros((row, column))
    for i in range(1, row-1):
        for j in range(1, column-1):
            window= img[i-1: i+2, j-1:j+2]
            product= window*structuring_ele
...
def morph_dilate(img):
    ...
    """
    row, column = img.shape 
    dilate_img = np.zeros((row, column))
    for i in range(1, row-1):
        for j in range(1, column-1):
            window = img[i-1:i+2, j-1:j+2]
            product = window*structuring_ele
...
def boundary(img):
    ...
    bound_img = np.subtract(img, bound_img)
    return bound_img
